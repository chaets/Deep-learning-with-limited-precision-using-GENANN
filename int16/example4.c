...
void load_data() {
    ...
    input = malloc(sizeof(int_fast16_t) * samples * 4);
    class = malloc(sizeof(int_fast16_t) * samples * 3);

    /* Read the file into our arrays. */
    int i, j;
    for (i = 0; i < samples; ++i) {
        int_fast16_t *p = input + i * 4;
        int_fast16_t *c = class + i * 3;
        c[0] = c[1] = c[2] = 0.0;
...
int main(int argc, char *argv[])
{
    ...
    int correct = 0;
    for (j = 0; j < samples; ++j) {
        const int_fast16_t *guess = genann_run(ann, input + j*4);
